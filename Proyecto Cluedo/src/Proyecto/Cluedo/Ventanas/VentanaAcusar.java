...
public class VentanaAcusar extends JFrame {
	...
	// ImageIcon vacio2 = new
	// ImageIcon(ventana.class.getResource("Imagenes/vacio.png"));
	// Icon iconovacio2 = new
	// ImageIcon(vacio2.getImage().getScaledInstance(hueco2.getWidth() ,
	// hueco2.getHeight(), Image.SCALE_DEFAULT));
	...
	// //meter botones
	// ImageIcon iconobacusar = new
	// ImageIcon(ventana.class.getResource("Imagenes/bacusar.png"));
	// lbAcusar.setSize(200,150);
	//
	// Icon iconoacu = new
	// ImageIcon(iconobacusar.getImage().getScaledInstance(lbAcusar.getWidth() ,
	// lbAcusar.getHeight(), Image.SCALE_DEFAULT));
	// lbAcusar.setIcon(iconoacu);
	// ImageIcon iconobresolver = new
	// ImageIcon(ventana.class.getResource("Imagenes/Bresolver.png"));
	// lbResolver.setSize(200,150);
	// Icon iconores = new
	// ImageIcon(iconobresolver.getImage().getScaledInstance(lbResolver.getWidth()
	// , lbResolver.getHeight(), Image.SCALE_DEFAULT));
	...
	// public void mousePressed(MouseEvent e) {
	// if(arposcartas[0]>-1 &&
	// arposcartas[0]<(prop.getBaraja().get(0).size()-1)){
	// arposcartas[0]=arposcartas[0]+1;
	// meterCartas(prop,0,pcarma,labela);
	...
	// public void mousePressed(MouseEvent e) {
	// if(arposcartas[2]>-1 &&
	// arposcartas[2]<(prop.getBaraja().get(2).size()-1)){
	// System.out.println(prop.getBaraja().get(2).size());
	// arposcartas[2]=arposcartas[2]+1;
	// meterCartas(prop,2,pcsospechoso,labels);
	//
	...
	//
	// }
	// public static void meterCartas(Propiedades p,int panel,JPanel pan,JLabel
	// label){
	// String
	// ruta=p.getBaraja().get(panel).get(arposcartas[panel]).getRutaIcono();
	...
	//
	// }
	// public static void cargarFlecha(JLabel lflecha,String ruta,JPanel
	// panel,int num){
	// lflecha.setSize(new Dimension(80,80));
	// System.out.println(ruta);
	// ImageIcon iilflecha = new
	// ImageIcon(VentanaAcusar.class.getResource(ruta));
	// Icon iconovacios = new
	// ImageIcon(iilflecha.getImage().getScaledInstance(lflecha.getWidth() ,
	// lflecha.getHeight(), Image.SCALE_DEFAULT));
	...
	public String[] resolver = new String[3];
	...
	public VentanaAcusar(GestionBaseDeDatos base, Connection con, Jugador j, Partida p, HiloTurno hTurno) {
		...
		// ImageIcon(ventana.class.getResource(ruta));
		// pprincipal=new panelrosa(imagefondo.getImage());
		...
		Icon iconoacu = new ImageIcon(iconobacusar.getImage().getScaledInstance(lbAcusar.getWidth(),
				lbAcusar.getHeight(), Image.SCALE_DEFAULT));
		...
		Icon iconores = new ImageIcon(iconobresolver.getImage().getScaledInstance(lbResolver.getWidth(),
				lbResolver.getHeight(), Image.SCALE_DEFAULT));
...
//							+ "cometio el crimen \n en" + acusacion[0] + "con  \n el" + acusacion[1], p);
		...
		lbResolver.addMouseListener(new MouseAdapter() {
...
//					resolver[0] ="Edificio de letras";
					...
					if (resultado[0].equals(resolver[0])) {
						if (resultado[1].equals(resolver[1])) {
							if (resultado[2].equals(resultado[2])) {

								JOptionPane.showMessageDialog(null,
										"¡ENHORABUENA! Has resuelto el misterio correctamente", "Ganador",
										JOptionPane.INFORMATION_MESSAGE);

								base.modificarGanador(con, j.getUsuario(), 1, p.getCodigo());
	...
	public void SubirAbaseAcusacion(Jugador j, Partida p, Connection conexion) {
		...
		try {
			...
			String sql = "INSERT INTO SOSPECHA VALUES ('" + acusacion[0] + "','" + acusacion[1] + "','" + acusacion[2]
					+ "'," + j.getCodigo() + "," + p.getCodigo() + "," + System.currentTimeMillis() + ")";
