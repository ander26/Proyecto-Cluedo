...
public class VentanaLogin extends JFrame {
	...
	public VentanaLogin(Connection conexion, GestionBaseDeDatos base) {
		...
		JLabel explicacion = new JLabel(
				"<html><body>¡Bienvenido!<br>Estas apunto de adrentarte en un mundo muy misterioso,<br>¿Seras capaz de convertirte en el nuevo Sherlock Holmes?<br> Entra y descubrelo</body></html>");
		...
		icono = new ImageIcon(imagen.getImage().getScaledInstance(imagenLogo.getWidth(), imagenLogo.getHeight(),
				Image.SCALE_DEFAULT));
		...
		iconoSalir = new ImageIcon(
				imagen.getImage().getScaledInstance(salir.getWidth(), salir.getHeight(), Image.SCALE_DEFAULT));
		...
		iconoEntrar = new ImageIcon(imagen.getImage().getScaledInstance(botonEntrar.getWidth(), botonEntrar.getHeight(),
				Image.SCALE_DEFAULT));
		...
		iconoRegistrar = new ImageIcon(
				imagen.getImage().getScaledInstance(registrar.getWidth(), registrar.getHeight(), Image.SCALE_DEFAULT));

		registrar.setIcon(iconoRegistrar);

		olvidar.setBounds(375, 457, 25, 25);

		try {
			imagen = new ImageIcon(VentanaLogin.class.getResource("Imagenes/forget.png").toURI().toURL());
		} catch (Exception e) {
			System.out.println("No se ha encontrado el archivo");
		}

		iconoOlvidar = new ImageIcon(
				imagen.getImage().getScaledInstance(olvidar.getWidth(), olvidar.getHeight(), Image.SCALE_DEFAULT));

		olvidar.setIcon(iconoOlvidar);

		olvidar.setToolTipText("Pulsa aqui si has olvidado la contraseña");

		// Añadimos al panel

		getContentPane().setLayout(null);

		getContentPane().add(olvidar);
		...
		salir.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int n = JOptionPane.showOptionDialog(getContentPane(), "¿Estas seguro de que quieres salir?", "Salir",
		...
		olvidar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String nombreUsuario = JOptionPane.showInputDialog(getContentPane(), "Introduzca el nombre de usuario:",
						"Recuperar contraseña", JOptionPane.INFORMATION_MESSAGE);
				if (nombreUsuario != null) {
					if (nombreUsuario.trim().length() == 0) {
						JOptionPane.showMessageDialog(getContentPane(),
								"Lo siento no ha introducido ningun nombre de usuario", "Aviso",
								JOptionPane.WARNING_MESSAGE);
					} else {

						ArrayList<Usuario> u = base.consultaATabla(conexion, " NOMBREUSUARIO='" + nombreUsuario + "'");

						if (u.size() != 1) {
							JOptionPane.showMessageDialog(getContentPane(),
									"Lo siento no se ha podido encontrar a ese usuario", "Aviso",
									JOptionPane.WARNING_MESSAGE);

						} else {

							String respuesta = JOptionPane.showInputDialog(getContentPane(),
									"Introduzca la respuesta a la siguiente pregunta: "
											+ u.get(0).getListaPreguntas()[u.get(0).getPregunta()],
									"Pregunta Seguridad", JOptionPane.INFORMATION_MESSAGE);
							if (respuesta != null) {
								if (respuesta.trim().length() == 0 || !(respuesta.equals(u.get(0).getRespuesta()))) {
									JOptionPane.showMessageDialog(getContentPane(),
											"No se ha podido verificar la identidad lo siento", "Aviso",
											JOptionPane.WARNING_MESSAGE);
								} else {
									JOptionPane.showMessageDialog(getContentPane(),
											"Su contraseña es : " + u.get(0).getContraseña(),
